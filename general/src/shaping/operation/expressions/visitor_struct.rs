...
#[allow(unused)]
pub trait ExpressionVisitorSim {
...
pub fn navigate_expression(visitor: Rc<RefCell<dyn ExpressionVisitorSim>>, input: &str) -> Result<Expression> {
    let expression = ActionParser::parse(Rule::action, input)?.next().unwrap();

    println!("{input}");

    fn parse_expression(visitor: Rc<RefCell<dyn ExpressionVisitorSim>>, pair: Pair<Rule>) -> Expression {
        match pair.as_rule() {
